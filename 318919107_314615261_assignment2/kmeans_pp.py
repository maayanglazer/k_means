...
def kmeans_pp(vectors, N, k, iter, eps):
    ...
    for i in range(len(vectors)):
        ...
        list_vectors.append(np_vectors[i].tolist())
    ...
    for i, ind in enumerate(chosen_indices):
        if i != len(chosen_indices) - 1:
            ...
            res_str += f"{ind}\n"
    chosen_vectors = [list_vectors[i] for i in chosen_indices]
    K_array = mykmeanssp.fit(K, iter, chosen_vectors, list_vectors, len(list_vectors[0]), len(list_vectors), eps)
