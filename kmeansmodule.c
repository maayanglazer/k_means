...
void fit(int K, int iter, double **k_array, double **vectors, int dim)
{
...
static PyObject *kmeans(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "di", &K, &iter, &k_array, &vectors, &dim))
    {
    ...
    return Py_BuildValue("d", fit(K, iter, k_array, vectors, dim)); /*  Py_BuildValue(...) returns a PyObject*  */
}
